...
selection behavior. It uses fixed service times and a seeded P2C choice so a
scenario can be repeated.
...
application-specific failure responses. Add those models only when a test
needs them; keep the basic scenarios small enough to explain when they fail.
